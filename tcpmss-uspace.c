...
uint16_t ip_checksum_finalize(uint32_t *tmp) { return ~(*tmp); }
...
static int packet_callback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
  ...
  if (ph) {
    ...
    dprintf("hw_protocol=0x%04x hook=%u id=%u ", ntohs(ph->hw_protocol),
            ph->hook, id);
  }
  ...
  if (payload_len < sizeof(struct iphdr)) {
    ...
    dprintf("ip version %u len %u proto %u csum %u ", iph->version, iph->ihl,
            iph->protocol, iph->check);

    if (iph->protocol == IPPROTO_TCP &&
        payload_len >= (iph->ihl * 4) + sizeof(struct tcphdr)) {
      struct tcphdr *tcph = (struct tcphdr *)(data + (iph->ihl * 4));
      dprintf("dst port=%u data_offset=%u syn=%u ", ntohs(tcph->dest),
              tcph->doff, tcph->syn);
